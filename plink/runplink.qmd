...
```
paulo@pop-os |> ~/Documents/GENOMICS/plink |> tree

```

**paulo@pop-os** : meu nome de usuario e nome da minha máquina

**~/Documents/GENOMICS/plink** : o caminho do diretório no qual estou trabalhando
...
```
paulo@pop-os |> ~/Documents/GENOMICS/plink |> cat toy.ped 
...
```
paulo@pop-os |> ~/Documents/GENOMICS/plink |> cat toy.map
1	rs0     0   1000
1	rs10    0   1001
```
...
```
paulo@pop-os |> ~/Documents/GENOMICS/plink |> ./plink 
...
```
✘ paulo@pop-os |> ~/Documents/GENOMICS/plink |> ./plink --file toy --freq --out toy_analysis
PLINK v1.90b7 64-bit (16 Jan 2023)             www.cog-genomics.org/plink/1.9/
(C) 2005-2023 Shaun Purcell, Christopher Chang   GNU General Public License v3
...
* `--freq`  :  este comando informa ao PLINK para gerar um relatório de frequências alélicas. A [{{< fa book size=lg >}}     documentação do plink](https://www.cog-genomics.org/plink/1.9/data) oferece a descrição de todas as funções disponíveis, e também as configurações dos arquivos de saída para cada função.
...
```
paulo@pop-os |> ~/Documents/GENOMICS/plink |> cat toy_analysis.frq
